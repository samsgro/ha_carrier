...
This branch is `2.28.2+oauthdiag.2`. It is a diagnostics-only fork of
...
diagnostic build that logs secret-safe OAuth refresh metadata and reads
the OAuth JSON/body before aiohttp `raise_for_status` can release it.

## Pins

| Item | Value |
| --- | --- |
| `ha_carrier` version | `2.28.2+oauthdiag.2` |
| `carrier-api` version | `3.6.0+oauthdiag.2` |
| Immutable `carrier-api` commit | `af96d9514a71a6050e6642e9e12d553bf7f41c08` |
...
`invalid_grant` still raises `CarrierApiAuthError`. Other HTTP 400 OAuth
errors remain `CarrierApiTokenRefreshError`. All HTTP 400 responses are
not treated as auth failures.
...
2. Confirm `manifest.json` `version` is `2.28.2+oauthdiag.2` and
...
4. Confirm the loaded package version is `3.6.0+oauthdiag.2`.
...
Expected print: `3.6.0+oauthdiag.2`.
